...
import { newResponse } from '../../../common/entities/response.entity';
import { ErrorCode } from '../../../common/error/error-code.enum';
import { Subject } from 'rxjs';

@Injectable()
export class ResponseParserMiddleware implements NestMiddleware {
  static createResSubject(request: Request): Subject<IResponseDto<any>> {
    request['resSubject'] =
      request['resSubject'] || new Subject<IResponseDto<any>>();
      ...
      if (hasResponseData) {
        ...
        if (isCached) {
          ...
          try {
            ...
            try {
              const res = body as IResponseDto<any>;
              msg = res.msg;
            } catch {}
          } catch {
            body = rawBody || {};
