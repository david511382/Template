...
export class RequestLoggerTemplate {
  static async getLogger(moduleRef: ModuleRef,contextId?:ContextId): Promise<LoggerService> {
    if (!contextId)
  ...
  static async getRequestLogger(moduleRef: ModuleRef,contextId:ContextId): Promise<LoggerService> {
  ...
  static async fetchContextId(moduleRef: ModuleRef):Promise<ContextId>{
    ...
    static async getDefaultLogger(moduleRef: ModuleRef):Promise<LoggerService>{
      ...
      protected get logger() :LoggerService {
        return Promise.apply(RequestLoggerTemplate.getLogger,[this._moduleRef,this.contextId]);
      }
